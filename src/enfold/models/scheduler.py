...
class ContinuousFlowMatchScheduler:
    """Continuous-time Flow-Matching scheduler with optional shift-based training sampling."""
    ...
    def __init__(
        ...
        shift: float = 5.0,
        eps: float = 1e-10,
        uniform_sigma_sampling: bool = False,
    ):
        ...
        self.eps = float(eps)
        self.uniform_sigma_sampling = bool(uniform_sigma_sampling)
    ...
    def _precompute_training_weight_stats(self) -> tuple[float, float]:
        ...
        sigma_grid = u_grid if self.uniform_sigma_sampling else self._phi(u_grid, self.shift)
        t_grid = sigma_grid * float(steps)
    ...
    def sample_training_t(self, batch_size: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        ...
        sigma = u if self.uniform_sigma_sampling else self._phi(u, self.shift)
