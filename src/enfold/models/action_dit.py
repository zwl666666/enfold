...
class ConcatConditionActionDiTBlock(nn.Module):
...
class ActionDiT(nn.Module):
    ...
    def _rope_cache_len(self) -> int:
        if isinstance(self.freqs, tuple):
            return int(self.freqs[0].shape[0])
    ...
    def pre_dit(
        ...
        rope_cache_len = self._rope_cache_len()
        if seq_len > rope_cache_len:
            raise ValueError(
                f"Action token length {seq_len} exceeds RoPE cache {rope_cache_len}."
            )
        ...
        context_len = int(context_emb.shape[1])
        total_len = context_len + seq_len
        if total_len > rope_cache_len:
            raise ValueError(
                f"Action concat length {total_len} exceeds RoPE cache {rope_cache_len} "
                f"(context={context_len}, action={seq_len})."
            )
        tokens = torch.cat([context_emb, tokens], dim=1)
        valid_tokens = torch.cat(
            [
                context_mask.to(device=tokens.device, dtype=torch.bool),
                torch.ones((batch_size, seq_len), dtype=torch.bool, device=tokens.device),
            ],
            dim=1,
        )
        context_attn_mask = valid_tokens[:, None, None, :].expand(-1, 1, total_len, -1)
        if isinstance(self.freqs, tuple):
            freqs = (
                self.freqs[0][:total_len].to(tokens.device),
                self.freqs[1][:total_len].to(tokens.device),
            )
        else:
    ...
    def post_dit(self, tokens: torch.Tensor, pre_state: Dict[str, Any]) -> torch.Tensor:
        context_len = int(pre_state["meta"].get("context_len", 0))
        if context_len > 0:
