...
def action_num_frames(action_chunk_len: Any) -> int:
    """Convert action chunk length to dataset `num_frames` (= chunk + 1)."""
    chunk_len = int(action_chunk_len)
    if chunk_len <= 0:
        raise ValueError(f"`action_chunk_len` must be positive, got {chunk_len}")
    return chunk_len + 1
...
def register_default_resolvers() -> None:
    ...
    _register("action_num_frames", action_num_frames)
