...
class EnfoldTrainer:
    ...
    def _load_weight_checkpoint_before_prepare(self):
        ...
        logger.info("Loading weight checkpoint only (before DeepSpeed prepare): %s", resume_path)
    ...
    def _resume_or_load_checkpoint(self):
        ...
        if resume_path.is_dir():
            ...
            return
        if resume_path.is_file() and self.accelerator.is_main_process:
            logger.warning(
    ...
    def evaluate(self):
        ...
        action_only = getattr(model, "teacher_video_expert", None) is None
        ...
        if action_only:
            pred_action = None
            if action is not None and hasattr(model, "infer_action"):
                action_infer_kwargs = {
                    "input_image": input_image,
                    "action_horizon": sample["action_horizon"],
                    "proprio": proprio,
        ...
        if action is not None and pred_action is not None:
            ...
            for action_name, raw_action in (("pred", pred_action), ("gt", action)):
                ...
                denorm_action = processor.denormalize_action(
                    action_btd,
                    base_proprio=base_proprio if processor.use_relative_joint_action else None,
                )
                if denorm_action.ndim == 2:
                    denorm_action = denorm_action.unsqueeze(0)
