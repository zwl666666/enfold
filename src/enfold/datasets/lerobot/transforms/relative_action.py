...
def resolve_action_state_transforms(
    shape_meta: Dict[str, Any],
    action_state_transforms: Optional[List[Any]],
    use_relative_joint_action: bool,
) -> Optional[List[Any]]:
    if action_state_transforms is not None and use_relative_joint_action:
        raise ValueError(
            "Set either `action_state_transforms` or `use_relative_joint_action=true`, not both."
        )
    if not use_relative_joint_action:
        return action_state_transforms
    action_keys = [meta["key"] for meta in shape_meta["action"]]
    return [RelativeJointTransform(keys=action_keys)]
