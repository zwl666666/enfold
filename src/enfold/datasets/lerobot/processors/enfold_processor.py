...
from enfold.utils.pytorch_utils import dict_apply
from enfold.utils.logging_config import get_logger
from .base_processor import BaseProcessor
from ..transforms.relative_action import resolve_action_state_transforms

logger = get_logger(__name__)

class EnfoldProcessor(BaseProcessor):
    def __init__(
        self,
        # keys
        ...
        self.use_relative_joint_action = bool(use_relative_joint_action)
        self.action_state_transforms = resolve_action_state_transforms(
            shape_meta,
            action_state_transforms,
            self.use_relative_joint_action,
        )
    ...
    def normalizer(self) -> LinearNormalizer:
        if self.video_only:
            raise ValueError("video-only processor does not use an action/state normalizer.")
    ...
    def denormalize_action(
        ...
        """Map normalized model actions back to env-space absolute joint targets.

        Args:
            action: ``[T, D]`` or ``[B, T, D]`` normalized action tensor.
            base_proprio: Normalized proprio used as the relative-action base frame.
                Required when ``use_relative_joint_action`` is enabled. Accepts
                ``[D]``, ``[B, D]``, ``[T, D]``, or ``[B, T, D]``; only the first
                time step is used.
        """
        if action.ndim == 2:
            action = action.unsqueeze(0)
        if action.ndim != 3:
            raise ValueError(f"`action` must be [T, D] or [B, T, D], got {tuple(action.shape)}")
        if self.use_relative_joint_action and base_proprio is None:
            raise ValueError(
                "`base_proprio` is required when `use_relative_joint_action` is enabled."
            )

        batch: Dict[str, Any] = {"action": action}
        if base_proprio is not None:
            base = base_proprio
            if base.ndim == 1:
            ...
            if base.ndim != 3:
                raise ValueError(
                    f"`base_proprio` must be [D], [B, D], [T, D], or [B, T, D], got {tuple(base.shape)}"
                )
            if base.shape[0] == 1 and action.shape[0] > 1:
                base = base.expand(action.shape[0], -1, -1)
            if base.shape[0] != action.shape[0]:
                raise ValueError(
                    f"`base_proprio` batch ({base.shape[0]}) must match action batch ({action.shape[0]})."
                )
            batch["state"] = base[:, :1, :].expand(-1, action.shape[1], -1)
