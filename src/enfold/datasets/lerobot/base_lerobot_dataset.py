...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.use_instruction_segments = bool(use_instruction_segments)
        self.instruction_segment_track = str(instruction_segment_track)
        ...
        self.instruction_segments = [
            meta.info.get("instruction_segments", {}) if self.use_instruction_segments else {}
            for meta in metas
        ]
        self.dataset_instruction_fallbacks = [
            self._collect_valid_segment_instructions(segments)
            for segments in self.instruction_segments
        ]
        ...
        self.episode_indices = None if episode_indices is None else [int(idx) for idx in episode_indices]

        self.image_meta = shape_meta["images"]
        self.state_meta = [] if self.video_only else shape_meta["state"]
        self.action_meta = [] if self.video_only else shape_meta["action"]
        ...
        for meta in metas:
            episode_indices = (
                ...
                else [idx for idx in self.episode_indices if 0 <= idx < meta.total_episodes]
            )
            if not episode_indices:
                raise ValueError(
                    f"No valid episode indices selected for dataset {meta.repo_id}. "
                    f"Got {self.episode_indices} with total_episodes={meta.total_episodes}."
                )
        ...
        self.multi_dataset = MultiLeRobotDataset(
            ...
            aggregate_metadata_stats=not self.video_only,
        )
    ...
    def _collect_valid_segment_instructions(self, episode_segments):
        preferred = []
        any_track = []
        for segments in episode_segments.values():
            for seg in segments:
                instruction = str(seg.get("instruction", "")).strip()
                if not instruction:
                    continue
                any_track.append(instruction)
                if seg.get("track") == self.instruction_segment_track:
                    preferred.append(instruction)
        return {
            "preferred": preferred,
            "any": any_track,
        }
    ...
    def _get_segment_instruction(self, lerobot_sample, fallback: str) -> str:
        if not self.use_instruction_segments:
            return fallback
        dataset_index = self._scalar_to_int(lerobot_sample.get("dataset_index", 0))
        if dataset_index < 0 or dataset_index >= len(self.instruction_segments):
            return fallback
        episode_index = self._scalar_to_int(lerobot_sample["episode_index"])
        frame_index = self._scalar_to_int(lerobot_sample["frame_index"])
        segments = self.instruction_segments[dataset_index].get(str(episode_index), [])

        candidates = [
            seg for seg in segments
            if seg.get("track") == self.instruction_segment_track
            and str(seg.get("instruction", "")).strip()
        ]
        if not candidates:
            candidates = [seg for seg in segments if str(seg.get("instruction", "")).strip()]
        nearest = None
        nearest_distance = None
        for seg in candidates:
            start = int(seg.get("start_frame_index", 0))
            end = int(seg.get("end_frame_index", seg.get("success_frame_index", start)))
            if start <= frame_index <= end:
                instruction = str(seg.get("instruction", "")).strip()
                return instruction
            distance = min(abs(frame_index - start), abs(frame_index - end))
            if nearest is None or distance < nearest_distance:
                nearest = seg
                nearest_distance = distance
        if nearest is not None:
            instruction = str(nearest.get("instruction", "")).strip()
            if instruction:
                return instruction
        fallback_pool = self.dataset_instruction_fallbacks[dataset_index]
        if fallback_pool["preferred"]:
            return fallback_pool["preferred"][0]
        if fallback_pool["any"]:
            return fallback_pool["any"][0]
        return fallback
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            "idx": sample_idx,
            "task": self._get_segment_instruction(lerobot_sample, str(lerobot_sample["task"])),
            "images": {},
        }
