...
class Processor:
    ...
    def __init__(self, name="Qwen/Qwen2.5-VL-3B-Instruct", cache_dir=None):
        # Allow loading tokenizer from a local path to avoid checkpoint_db/HF download.
        local_path = Path(name).expanduser()
        self.is_local_name = local_path.exists()
        if self.is_local_name:
            name = str(local_path.resolve())
        self.name = name

        if not self.is_local_name and name not in [
        ...
        if self.is_local_name:
            cache_dir = str(local_path.resolve())
            self.processor = AutoProcessor.from_pretrained(cache_dir)
            log.info(f"Successfully loaded processor from local path: {cache_dir}")
        else:
    ...
    def apply_chat_template(
        ...
        if self.name.startswith("Qwen/Qwen2") or self.is_local_name:
