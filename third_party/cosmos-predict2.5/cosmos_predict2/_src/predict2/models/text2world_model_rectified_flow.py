...
class Text2WorldModelRectifiedFlow(ImaginaireModel):
    ...
    def state_dict(self, destination=None, prefix: str = "", keep_vars: bool = False) -> Dict[str, Any]:  # noqa: F821
        net_state_dict = self.net.state_dict(destination=destination, prefix=prefix + "net.", keep_vars=keep_vars)
        if self.config.ema.enabled:
            ema_state_dict = self.net_ema.state_dict(prefix=prefix + "net_ema.", keep_vars=keep_vars)
