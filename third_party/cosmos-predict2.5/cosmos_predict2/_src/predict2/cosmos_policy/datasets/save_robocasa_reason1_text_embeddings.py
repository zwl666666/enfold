...
This is the Reason1 equivalent of save_robocasa_t5_text_embeddings.py, used for
rectified flow models (predict2.5) which require Reason1 embeddings instead of T5.

Usage:
    uv run -m cosmos_predict2._src.predict2.cosmos_policy.datasets.save_robocasa_reason1_text_embeddings \
        --data_dir /path/to/robocasa/data

    # With custom options:
    uv run -m cosmos_predict2._src.predict2.cosmos_policy.datasets.save_robocasa_reason1_text_embeddings \
        --data_dir /path/to/robocasa/data \
        --rollout_data_dir /path/to/robocasa/rollout_data \
        --batch_size 4 \
...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/lustre/fsw/portfolios/dir/users/user/data/robocasa/robocasa_regen_v2_1199succDemos/",
        help="Directory containing RoboCasa dataset (HDF5 files)",
    )
    parser.add_argument(
        "--rollout_data_dir",
        type=str,
...
def main():
    ...
    print("=" * 60)
    print("RoboCasa Reason1 Embedding Precomputation")
    print("=" * 60)
    print(f"Data directory: {args.data_dir}")
    print(f"Rollout data directory: {args.rollout_data_dir}")
