...
- Check or create the relative symlink:
  `RoboTwin/policy/enfold_policy -> experiments/robotwin/enfold_policy`.
- Forward config overrides to the official RoboTwin entrypoint
  `script/eval_policy.py` and save logs.

Common arguments:
- `ckpt`: path to the Enfold checkpoint (required).
- `EVALUATION.task_name`: task name to evaluate (required).
- `gpu_id`: selects a logical device from an inherited `CUDA_VISIBLE_DEVICES`
  mask, or a physical device when no mask is set.
...
def _ensure_policy_symlink(robotwin_root: Path, policy_source_dir: Path) -> Path:
    ...
    if not policy_target.exists() and not policy_target.is_symlink():
        relative_source = os.path.relpath(source_resolved, start=policy_root.resolve())
...
def _collect_policy_config_overrides(raw_overrides: Iterable[str]) -> list[str]:
    policy_overrides: list[str] = []
    for raw in raw_overrides:
        key = raw.split("=", 1)[0].lstrip("+~")
        if key == "task":
            continue
        if key.startswith(("model.", "data.")):
            policy_overrides.append(str(raw))
...
def main(cfg: DictConfig):
    ...
    sim_overrides = _collect_policy_config_overrides(HydraConfig.get().overrides.task)
    ...
    _append_override(overrides, "sim_overrides", sim_overrides)
    ...
    _append_override(overrides, "action_trt_path", cfg.EVALUATION.action_trt_path)
    _append_override(overrides, "student_trt_path", cfg.EVALUATION.student_trt_path)
    _append_override(
        overrides,
    ...
    if inherited_visible_devices:
        if not 0 <= logical_gpu_id < len(inherited_visible_devices):
            raise ValueError(
                f"gpu_id={logical_gpu_id} is outside the inherited CUDA_VISIBLE_DEVICES "
                f"mask {inherited_visible_devices}."
            )
        env["CUDA_VISIBLE_DEVICES"] = inherited_visible_devices[logical_gpu_id]
    else:
        env["CUDA_VISIBLE_DEVICES"] = str(logical_gpu_id)
