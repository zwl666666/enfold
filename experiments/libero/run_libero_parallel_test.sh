...
run_libero_eval() {
    ...
    export ROOT_DIR
    # Fix MuJoCo EGL import crash in conda workers.
    if [[ -f "${ROOT_DIR}/scripts/libero_egl_env.sh" ]]; then
        # shellcheck source=/dev/null
        source "${ROOT_DIR}/scripts/libero_egl_env.sh"
    fi
    ...
    # Same interpreter as the Hydra manager (run_libero_manager sets PYTHON=sys.executable).
    WORKER_PYTHON="${PYTHON:-python}"
    ...
    echo "CKPT: $CKPT"
    echo "CONFIG: $CONFIG"
    echo "WORKER_PYTHON: $WORKER_PYTHON"
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && cd $ROOT_DIR && source $ROOT_DIR/scripts/libero_egl_env.sh && export EXP_NAME=$EXP_NAME && export PYTHONPATH=$ROOT_DIR/src:\${PYTHONPATH:-} && sleep $((gpu_id)) && \
            STATUS_FILE='$status_file' LOG_FILE='$log_file' RESULT_FILE='$result_file' && \
            CUDA_VISIBLE_DEVICES=$gpu_id MUJOCO_EGL_DEVICE_ID=$gpu_id $WORKER_PYTHON experiments/libero/eval_libero_single.py \
            task=$CONFIG ckpt=$CKPT \
            EVALUATION.task_suite_name=$suite EVALUATION.task_id=$task_id gpu_id=$gpu_id \
            EVALUATION.num_trials=$NUM_TRIALS EVALUATION.output_dir=$OUTPUT_DIR EVALUATION.device=cuda $EXTRA_ARGS > \"\$LOG_FILE\" 2>&1; \
            rc=\$?; \
    ...
    $WORKER_PYTHON experiments/libero/summarize_results.py --output_dir="$OUTPUT_DIR"
}
