...
from enfold.utils.pytorch_utils import set_global_seed
...
def _denormalize_action(action: torch.Tensor, processor: EnfoldProcessor) -> np.ndarray:
    if action.ndim == 2:
...
def _validate_visualize_future_video_cfg(cfg: DictConfig) -> None:
    if not bool(cfg.EVALUATION.get("visualize_future_video", False)):
        return
    if bool(cfg.model.get("action_only_eval", False)):
        raise ValueError(
            "EVALUATION.visualize_future_video=true requires model.action_only_eval=false "
            "because future-video prediction needs the Video DiT."
        )
...
def _predict_action_chunk(
    ...
    *,
    ...
    observation_horizon = int(cfg.model.student_config.get("observation_horizon", 1))
    if observation_horizon > 1 and observation_history:
        stride = max(1, int(observation_stride))
        history = observation_history[::-stride][:observation_horizon][::-1]
        if len(history) < observation_horizon:
            history = [history[0]] * (observation_horizon - len(history)) + history
        image = torch.stack([frame.squeeze(0) for frame in history], dim=1).unsqueeze(0)
...
def run_single_episode(
    ...
    *,
    ...
    observation_horizon = int(cfg.model.student_config.get("observation_horizon", 1))
    observation_stride = int(cfg.data.train.get("action_video_freq_ratio", 1))
    observation_history: list[torch.Tensor] = []
    use_observation_history = observation_horizon > 1
    ...
    while t < max_steps + num_steps_wait:
        ...
        if use_observation_history:
            history_image, _, _ = _obs_to_model_input(
                obs,
                cfg=cfg,
                processor=processor,
                width=input_w,
            ...
            observation_history.append(history_image.detach())
            max_history_len = (observation_horizon - 1) * max(1, observation_stride) + 1
            if len(observation_history) > max_history_len:
                observation_history = observation_history[-max_history_len:]
        ...
        if len(pending_actions) == 0:
            action_chunk, imgs, predicted_future_frames = _predict_action_chunk(
                ...
                observation_history=observation_history if use_observation_history else None,
...
def eval_single_process(cfg: DictConfig):
    ...
    processor: EnfoldProcessor = instantiate(cfg.data.train.processor).eval()
    processor.set_normalizer_from_stats(dataset_stats)
    logging.info("Using dataset stats: %s", dataset_stats_path)

    action_horizon_cfg = cfg.EVALUATION.get("action_horizon", None)
    if action_horizon_cfg is None:
        action_horizon = int(cfg.get("action_chunk_len", cfg.data.train.num_frames - 1))
    else:
