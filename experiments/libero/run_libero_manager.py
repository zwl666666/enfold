...
def _resolve_worker_task_choice() -> str:
    ...
    if task_choice is None or str(task_choice).strip() == "":
        raise ValueError(
            "Hydra task choice is empty. Please pass task=... (e.g., task=enfold_libero)."
        )
...
def run_evaluation(
    *,
    ...
    pythonpath = os.pathsep.join(
        part for part in [str(Path(root_dir) / "src"), os.environ.get("PYTHONPATH", "")] if part
    )
    ...
    for libc_path in (
        ...
        if os.path.isfile(libc_path):
            ...
            if libc_path not in preload.split(":"):
                env["LD_PRELOAD"] = f"{libc_path}:{preload}" if preload else libc_path
            env.setdefault("LIBERO_EGL_LD_PRELOAD", libc_path)
            break
    env.update(
        {
            "CONFIG": task_choice,
            ...
            # tmux workers source ~/.bashrc and may not activate conda; pin the interpreter.
