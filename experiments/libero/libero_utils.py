...
from enfold.utils.video_io import save_mp4
...
def _normalize_single_visible_egl_device() -> None:
    """Use the worker-local EGL device when CUDA_VISIBLE_DEVICES exposes one GPU."""
    visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES", "")
    if "," in visible_devices or visible_devices.strip() == "":
        return
...
def get_libero_env(task, resolution, seed, env_num=1):
    ...
    retries = max(1, int(os.environ.get("LIBERO_ENV_CREATE_RETRIES", "8")))
    delay_sec = float(os.environ.get("LIBERO_ENV_CREATE_RETRY_DELAY_SEC", "2.0"))

    if env_num > 1:
        _normalize_single_visible_egl_device()
        ...
        for attempt in range(retries):
            try:
                ...
                msg = str(e)
                transient = (
                    "MUJOCO_EGL_DEVICE_ID" in msg
                    or "EGL_NO_DISPLAY" in msg
                    or "Cannot initialize a EGL device display" in msg
                    or "eglGetPlatformDisplayEXT" in msg
                )
                ...
                raise
        if env is None:
            raise RuntimeError(f"LIBERO env creation failed after {retries} tries") from last_err
    env.seed(
        seed
