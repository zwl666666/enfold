...
echo "[launch] cwd=$(pwd) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>}"
if command -v nvidia-smi >/dev/null 2>&1; then
  GPU_COUNT="$(nvidia-smi -L 2>/dev/null | wc -l | tr -d ' ')"
  echo "[launch] nvidia_gpu_count=${GPU_COUNT}"
  if is_integer "${GPU_COUNT}" && is_integer "${NPROC_PER_NODE}" && (( GPU_COUNT < NPROC_PER_NODE )); then
    echo "Warning: nproc_per_node=${NPROC_PER_NODE} exceeds visible GPU count=${GPU_COUNT}. Processes may pile onto the same device." >&2
  fi
fi
